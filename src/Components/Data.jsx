...
function Data() {
  ...
  const [capsuleType, setCapsuleType] = useState("dragon2");
  ...
  return (
    <Container>
      <Header>
        <h1>Get to know us a lill better!</h1>
        <div>
          <Select
            value={status}
            onChange={setStatus}
          ...
          <Select
            value={capsuleType}
            onChange={setCapsuleType}
            label="Select type"
            placeholder="Pick one"
            data={[
              { value: "dragon2", label: "Dragon 2.0" },
              { value: "dragon1", label: "Dragon 1.0" },
              { value: "dragon11", label: "Dragon 1.1" },
            ]}
          />
        </div>
      </Header>
              ...
              {data.map((item, index) => {
                return (
                  <tr>
                    ...
                    <td>{(new Date(item.original_launch)).toLocaleDateString("en-GB")}</td>
                    <td>{formatUnixTime(item.original_launch_unix)}</td>
                    <td>{item.type}</td>
